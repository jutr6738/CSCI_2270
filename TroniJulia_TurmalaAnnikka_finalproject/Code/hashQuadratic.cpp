...
//if index is full, checks index + i^2 until empty index is found
bool HashTable::insertItem(int key)
{
  ...
  if(table[index] == NULL){
    ...
    numOfcolision++;
    int i = 0;
    while(table[(index + (int)pow(i, 2)) % tableSize] != NULL){
      i++;
    }
    table[(index + (int)pow(i, 2)) % tableSize] = createNode(key, nullptr);
    return true;
  }
  return false;
}
...
//incriments i for (index + i^2) until the node's key at (index + i^2) equals
//argument, returns node at this index.
node* HashTable::searchItem(int key)
{
  unsigned int index = hashFunction(key);
  int i = 0;
  while(table[(index + (int)pow(i,2)) % tableSize]->key != key){
    numSearchCollision++;
    i++;
  }
  return table[(index + (int)pow(i,2)) % tableSize];
}
