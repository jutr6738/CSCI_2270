...
void Graph::removeEdge(int key1, int key2)
{
  ...
  // Ex: say you need to remove ith index item from a vector "myvector", you can use myvector.erase(myvector.begin()+i)
    for (int i=0; i< v1->adj.size(); i++)
    {
        if (v1->adj[i].v==v2)
        {
            v1->adj.erase(v1->adj.begin()+i);   //erasing v2 from v1's adj list 
            break;
        }
     ...
     for (int i=0; i< v2->adj.size(); i++)
    {
        if (v2->adj[i].v==v1)
        {
            v2->adj.erase(v2->adj.begin()+i);   //erasing v1 from v2's adj list 
            break;
        }
...
bool Graph::isBridge(int key1, int key2)
{
  ...
  {
      if (vertices[i]->visited ==false) //if the vertex is unvisited 
      {
          DFTraversal(vertices[i]); //call traversal
          initial_components+=1; //increment everytime dft is called (so it will only get called if you don't visit all the nodes from the starting vertex's adj list)
      }
  }

  cout<< "num. of connected components before removal: " << initial_components << endl;

  //Step 2: Remove the edge (key1,key2) from the grapph by modifying the adjacency list(s) in the removeEdge function.
  ...
  //Step 3: Get components_after_removal (use DFTraversal again)
    for (int i=0; i<vertices.size(); i++)  //iteratve over all vertices
  {
      if (vertices[i]->visited ==false) //if the vertex is unvisited 
      {
          DFTraversal(vertices[i]); //call traversal
          components_after_removal+=1; //increment everytime dft is called (so it will only get called if you don't visit all the nodes from the starting vertex's adj list)
      }
  }
  ...
  // Step5: check if the no of connected components increases after removing the edge and return true
